//! place — the IR → millimetre engine: gather, grid assignment, the greedy/
//! anneal placement strategies + cost, the elbow wiring, and SchematicWriter
//! assembly. The cohesive core — placement is cost-driven (scoring a candidate
//! routes the whole sheet), so place/cost/wire stay one module.
...
pub(super) fn grid_from_layout(design: &Design) -> BTreeMap<String, [i32; 4]> {
...
pub(super) fn gather(env: &KicadEnv, design: &Design) -> io::Result<Vec<Item>> {
