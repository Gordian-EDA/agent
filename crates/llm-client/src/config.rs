...
//! provider credentials (AWS Bedrock bearer token, OpenAI-compatible API
//! key + base URL, or an Anthropic API key).
//!
//! Two backends sit behind the [`crate::Provider`] trait: AWS Bedrock
//! ([`Config`]) and any OpenAI-compatible chat endpoint ([`OpenAiConfig`]).
//! [`selected_backend`] decides which to build. (A native Anthropic Messages
//! backend will be added behind the same trait via an adopted crate.)
...
pub fn provider_status() -> (String, String) {
    ...
    let model = match backend {
        Backend::Bedrock => {
            Config::from_env().map(|c| c.model).unwrap_or_else(|_| DEFAULT_MODEL.to_string())
        }
