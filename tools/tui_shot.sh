...
set -euo pipefail
cd "$(dirname "$0")/.."
cargo test -p gordian --bin gordian tui::screenshot::tui_screenshots -- --nocapture >/dev/null 2>&1 || {
  echo "test failed; rerun verbosely:"; cargo test -p gordian --bin gordian tui::screenshot::tui_screenshots; exit 1; }
