//! Run the `ClusterPlace` placement leaf standalone over the golden problem corpus.
//!
//! `cargo run -p cluster-place --example bench [--release] [name-substring]` needs no
...
fn main() {
    ...
    for (name, mut problem) in sch_model::golden_problems() {
        ...
        let t0 = Instant::now();
        cluster_place::ClusterPlace.place(&mut problem, &eval);
