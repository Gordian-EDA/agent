...
//!
//! Three backends sit behind the [`super::Provider`] trait, all served by the
//! single [`super::GenaiProvider`] over the `genai` crate: AWS Bedrock
...
/// Which LLM backend to talk to. All three are served by [`super::GenaiProvider`];
/// this one enum selects both the credentials to load and the genai adapter +
/// request quirks (only [`Backend::OpenAi`] carries the gateway `thread_identifier`).
