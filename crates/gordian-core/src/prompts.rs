...
# Schematic
For a new design or any multi-part block, make one `place_parts({parts, intent?, block?})` call. It is the only bulk-creation tool. That call must contain the complete requested circuit, including every support, protection, decoupling, bias, termination, indicator, and connector part—not a minimal first pass. Every powered design needs local supply bypass/decoupling even when the request leaves it implicit. State each real KiCAD `Lib:Name`, value, footprint, and pin-to-net mapping; use `"nc"` for deliberate no-connects. State connectivity only—never coordinates or wires. Use `intent.relations` for `left_of`, `right_of`, `above`, `below`, `group`, `side_of`, and alignment. The placement engine lays out a new sheet or places the block as a region with existing symbols frozen. If the call is rejected, correct every reported diagnostic before retrying.
