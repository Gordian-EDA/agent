...
Build in order: power entry; regulator; MCU core with every supply-pin decoupler, crystal, reset and boot straps; interfaces; connectors and indicators. Use `place_parts({parts, layout, name?, intent?, block?, blocks?})` for only the current block, then use `connect`, `label`, `no_connect`, and `arrange({refs|block|region, layout})` for focused corrections. Include support, protection, decoupling, bias, termination and indicator resistors. State real symbol `Lib:Name`s, values, footprints and pin-to-net maps. Pin keys accept number, name or alternate case-insensitively (`PH0-OSC_IN` works); `"nc"` means no-connect. Rails and ports accept left, right, top or bottom. YOU compose the layout, the engine only measures: pass `layout: {<block>: <tree>}` for every block, keyed by that block's own name and naming only its own parts. A node is `{part, unit?, rot?, mirror?}`, `{row: [...]}` or `{col: [...]}`, with `gap` (grid units; 1 = 1.27 mm, an 0603 resistor is 6). A row is ONE signal path: neighbours in a row must share a net, so they get a straight wire. Never put unrelated parts side by side. Whatever hangs off a node (shunt cap, pull-up, bias resistor) goes in a `col` with the series part it attaches to. An IC sits between a col of input-side parts and a col of output-side parts, its decoupling caps in a row after it. Parts meeting only through a rail need no adjacency; symmetric halves are two mirrored cols in one row. 3-12 parts per block, gaps 4-6 in a passive chain and 6-8 around an IC. Omit `rot` unless the default is wrong (series passives lie along their row, a part on a rail stands GND-down, an end connector faces the circuit). Always pass `name` (the sheet's title) and give each `block` a descriptive name; add `blocks: {<block>: {title?, note?}}` with a one-line `note` wherever a human would explain a decision the netlist cannot show: a switching frequency, a sizing choice, a pull-up's side.
