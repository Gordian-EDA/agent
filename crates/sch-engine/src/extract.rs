...
fn xy(n: &Sexp) -> [f64; 2] {
    ...
    [l.get(1).and_then(|v| v.as_f64()).unwrap_or(0.0), l.get(2).and_then(|v| v.as_f64()).unwrap_or(0.0)]
}

fn at_of(n: Option<&Sexp>) -> [f64; 3] {
    let Some(l) = n.and_then(|n| n.as_list()) else { return [0.0, 0.0, 0.0] };
    [
...
fn justify_of(effects: Option<&Sexp>) -> String {
    let Some(j) = effects.and_then(|e| e.child("justify")) else { return String::new() };
    j.as_list().unwrap()[1..].iter().map(|a| a.text()).collect::<Vec<_>>().join(" ")
}

/// `([x, y, rot, justify], hidden)` of a named property.
fn prop_pos(node: &Sexp, name: &str) -> (Option<Vec<serde_json::Value>>, bool) {
    let Some(p) = node.prop(name) else { return (None, false) };
    let eff = p.child("effects");
    let hidden = eff.and_then(|e| e.child("hide")).map(|h| h.as_list().unwrap()[1].text() == "yes").unwrap_or(false);
    let v = p.child("at").map(|a| {
        let a = at_of(Some(a));
        vec![json!(a[0]), json!(a[1]), json!(a[2] as i64), json!(justify_of(eff))]
    });
    (v, hidden)
}
...
fn text_of(n: Option<&Sexp>) -> String {
    n.and_then(|n| n.as_list()).and_then(|l| l.get(1)).map(|v| v.text()).unwrap_or_default()
}
...
pub fn parse(text: &str) -> Result<Design> {
    ...
    let mut des = Design { uuid: text_of(root.child("uuid")), ..Default::default() };
    ...
    if let Some(tb) = root.child("title_block") {
        ...
        for c in tb.children("comment") {
            des.comments.push(c.as_list().and_then(|l| l.get(2)).map(|v| v.text()).unwrap_or_default());
        }
    }
    ...
    let infos: HashMap<String, SymbolInfo> =
        des.lib_symbols.iter().map(|(k, v)| (k.clone(), parse_symbol_node(v, k))).collect();
    ...
    for node in &root.as_list().unwrap()[1..] {
        ...
        match node.tag() {
            "symbol" => {
                ...
                let rot = at[2] as i32;
                let mirror = node.child("mirror").map(|m| text_of(Some(m))).unwrap_or_default();
                let unit = node.child("unit").and_then(|u| u.as_list()?.get(1)?.as_f64()).unwrap_or(1.0) as i32;
                let uuid = node.child("uuid").map(|u| text_of(Some(u))).unwrap_or_default();
                ...
                let lib_name = node.child("lib_name").map(|n| text_of(Some(n))).unwrap_or_default();
                let info = infos.get(&lib_name).or_else(|| infos.get(&lib_id));
                let is_power = info.map(|i| i.power).unwrap_or_else(|| lib_id.starts_with("power:"))
                ...
                if is_power {
                    ...
                    let net = if value.is_empty() {
                        lib_id.split_once(':').map(|(_, n)| n.to_string()).unwrap_or_else(|| lib_id.clone())
                    } else {
                        value.clone()
                    };
                ...
                let mut part = Part {
                    ...
                    dnp: node.child("dnp").map(|d| text_of(Some(d)) == "yes").unwrap_or(false),
                    uuid,
                    lib_name,
                    ref_at,
                    val_at,
                ...
                for pr in node.children("property") {
                    ...
                    if matches!(name.as_str(), "Reference" | "Value" | "Footprint") || name.starts_with("ki_") {
                        continue;
                    }
                    let v = pr.as_list().and_then(|l| l.get(2)).map(|v| v.text()).unwrap_or_default();
                    if !v.is_empty() && v != "~" {
            ...
            "wire" => {
                des.wires
                    .push(node.child("pts").map(|p| p.children("xy").into_iter().map(xy).collect()).unwrap_or_default());
                des.wire_uuids.push(node.child("uuid").map(|u| text_of(Some(u))).unwrap_or_default());
            }
            tag @ ("label" | "global_label" | "hierarchical_label") => {
                ...
                des.labels.push(Label {
                    ...
                    .into(),
                    shape: node.child("shape").map(|s| text_of(Some(s))).unwrap_or_else(|| "input".into()),
                    uuid: node.child("uuid").map(|u| text_of(Some(u))).unwrap_or_default(),
            ...
            "text" => {
                ...
                if let Some(f) = eff.and_then(|e| e.child("font")) {
                    if let Some(sz) = f.child("size") {
                        size = sz.as_list().and_then(|l| l.get(1)?.as_f64()).unwrap_or(1.27);
                    }
                    bold = f.child("bold").map(|b| text_of(Some(b)) == "yes").unwrap_or(false);
                }
                ...
                des.texts.push(Text {
                    ...
                    size,
                    bold,
                    justify: if j.is_empty() { "left bottom".into() } else { j },
                    uuid: node.child("uuid").map(|u| text_of(Some(u))).unwrap_or_default(),
                });
            }
