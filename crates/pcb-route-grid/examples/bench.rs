//! Stand-alone grid-router bench: route the golden boards in `fixtures/` and
//! print wall time plus the leaf's own metrics.
//!
//! `cargo run -p pcb-route-grid --example bench` — no agent, no KiCAD, no
//! network. The design-rule oracle is injected here, exactly as the composition
//! root injects it in production.
...
use pcb_route_grid::router::GridRouter;
...
fn main() {
    let drc = StandardDrc;
    let router = GridRouter::new(&drc);
    println!(
        "{:<10} {:>5} {:>9} {:>7} {:>6} {:>7} {:>11} {:>5}",
        "board", "nets", "ms", "traces", "vias", "failed", "wirelength", "drc"
    );
    for name in ["quad.json", "led-r.json"] {
        let view = load(name);
