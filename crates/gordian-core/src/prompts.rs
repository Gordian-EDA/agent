...
Work one sub-circuit at a time, 3-12 parts: `place_parts({parts, layout, name?, intent?})` places and wires it as one group beside the sheet's content; `arrange({refs|bbox, layout})` corrects it. Then `create_and_update_block({parts, title})` outlines that group and writes its title — the parts must stand alone (drawn wires stay inside; sub-circuits meet only through net labels) — and, once every block exists, `arrange_blocks({rows: [[title, ...], ...]})` tiles the blocks as a grid, rows top to bottom, blockless parts under it; call it again after later edits. Compose 2-6 functional blocks (POWER, MCU, USB...), each holding ALL of one sub-circuit's parts; a crystal with its caps is never a block alone. Pin keys accept number, name or alternate, any case; `"nc"` means no-connect. Rails and ports take left/right/top/bottom. YOU compose the layout, the engine only measures: `layout` is one row/col tree over the payload's parts. A node is `{part, unit?, rot?, mirror?}`, `{row: [...]}` or `{col: [...]}`, with `gap` and `margin` ({left,top,right,bottom} or a number), in 1.27 mm grid units. A row is ONE signal path: neighbours must share a net; unrelated parts never sit side by side. A part that SERVES one pin — decoupler, pull-up, reset cap, shunt — goes in the `col` beside that part, on the side that pin leaves from; anywhere else needs a label. An IC sits between its input- and output-side cols, its decoupling caps a further col. Pass `name` (sheet title).
