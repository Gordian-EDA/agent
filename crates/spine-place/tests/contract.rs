//! The `SpinePlace` leaf's contract: deterministic given a seed, deadline-honouring, and
//! sound about what it returns. Runs on the golden problem corpus with a stub evaluator,
//! so `cargo test -p spine-place` needs neither KiCAD nor the realiser.
...
fn place(problem: &mut SchematicPlaceProblem) -> Vec<Pose> {
    ...
    spine_place::SpinePlace.place(problem, &StubEvaluator::new(&inc, &ir));
    problem
        .items
        .iter()
        .map(|it| {
