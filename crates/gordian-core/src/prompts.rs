...
# Schematic
For a new design or any multi-part block, make one `place_parts({parts, name?, intent?, block?})` call. It is the only bulk-creation tool. That call must contain the complete requested circuit, including every support, protection, decoupling, bias, termination, indicator, and connector part—not a minimal first pass. Give each named function the parts it really takes: one bypass capacitor per IC supply pin, a pull-up or pull-down on every bus, strap and enable line, a series resistor per indicator, bus protection as the pair the differential pair needs, and a split termination network—never one part standing in for a whole block. Name the sheet with `name`. State each real KiCAD `Lib:Name`, value, footprint, and pin-to-net mapping; use `"nc"` for deliberate no-connects. State connectivity only—never coordinates or wires. Use `intent.relations` for `left_of`, `right_of`, `above`, `below`, `group`, `side_of`, and alignment. The placement engine lays out a new sheet or places the block as a region with existing symbols frozen. If the call is rejected, correct every reported diagnostic before retrying. Before finishing, re-read the request block by block and count what each one owes; if any part of it is missing, call `place_parts` again with just the missing parts — a later call adds to the same sheet and leaves the placed symbols alone.
