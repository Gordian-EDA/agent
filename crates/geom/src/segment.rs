...
/// The polyline a board editor draws from `from` to `to`: a straight run along
/// one axis, then a single 45° leg onto the target.
///
/// `from` is the constrained end (a pad centre, an off-lattice terminal), so the
/// axial run leaves *it* — copper exits a pad square-on and turns once. Every
/// segment returned is octilinear by construction; an already-octilinear pair
/// comes back as the bare `[from, to]`.
pub fn octilinear_path(from: Point2, to: Point2) -> Vec<Point2> {
    let (dx, dy) = (to.x - from.x, to.y - from.y);
    let (ax, ay) = (dx.abs(), dy.abs());
    if ax < EPS || ay < EPS || (ax - ay).abs() < EPS {
        return vec![from, to];
    }
    let knee = if ax > ay {
        Point2::new(to.x - dx.signum() * ay, from.y)
    } else {
        Point2::new(from.x, to.y - dy.signum() * ax)
    };
    vec![from, knee, to]
}

#[cfg(test)]
mod octilinear_path_tests {
    use super::*;

    fn octilinear(a: Point2, b: Point2) -> bool {
        let (dx, dy) = ((b.x - a.x).abs(), (b.y - a.y).abs());
        dx < 1e-9 || dy < 1e-9 || (dx - dy).abs() < 1e-9
    }

    fn check(from: Point2, to: Point2) -> Vec<Point2> {
        let path = octilinear_path(from, to);
        assert_eq!(path.first(), Some(&from));
        assert_eq!(path.last(), Some(&to));
        for w in path.windows(2) {
            assert!(octilinear(w[0], w[1]), "{:?} -> {:?}", w[0], w[1]);
        }
        path
    }

    #[test]
    fn already_octilinear_pairs_stay_two_points() {
        for to in [
            Point2::new(5.0, 0.0),
            Point2::new(0.0, -3.0),
            Point2::new(2.0, 2.0),
            ...
            assert_eq!(check(Point2::new(0.0, 0.0), to).len(), 2);
        }
    }

    #[test]
    fn a_shallow_offset_leaves_along_the_long_axis() {
        let path = check(Point2::new(0.0, 0.0), Point2::new(10.0, 3.0));
        assert_eq!(path.len(), 3);
        assert_eq!(path[1], Point2::new(7.0, 0.0));
    }

    #[test]
    fn a_steep_offset_leaves_along_the_short_axis() {
        let path = check(Point2::new(0.0, 0.0), Point2::new(-3.0, -10.0));
        assert_eq!(path.len(), 3);
        assert_eq!(path[1], Point2::new(0.0, -7.0));
    }

    #[test]
    fn every_quadrant_and_aspect_stays_octilinear() {
        for i in -20..=20 {
            for j in -20..=20 {
                check(
                    Point2::new(1.234, -5.678),
                    Point2::new(1.234 + i as f64 * 0.37, -5.678 + j as f64 * 0.11),
                );
            }
        }
    }
