...
pub fn tool_defs() -> Vec<Tool> {
    ...
    let defs: Vec<(&str, &str, Value)> = vec![
        (
            "place_parts",
            "The ONLY way to create a new design or add a multi-part block. Submit the COMPLETE electrically finished block in one call, including every requested support, protection, decoupling, bias, termination, indicator, and connector part. State connectivity only: real KiCAD parts and pin-to-net mappings, never coordinates or wires. Before placement, the complete payload is validated for dangling pins and writes nothing on failure: every new named pin must land on a net with at least one other pin across the payload and existing sheet. One valid call lays out the whole new sheet, or places the block as a region while freezing existing symbols. The result's `gaps` are deterministic missing-support findings; for a complete powered/interface design, add the listed parts in one coherent follow-up place_parts call. When `gaps` is empty and the embedded schematic check is clean, do not resubmit or repair the block. They are advisory for deliberately minimal designs and focused edits. Use `intent.relations` for left_of/right_of/group/side_of placement. If rejected, correct every reported diagnostic before retrying; unknown-pin errors list valid physical pins.",
