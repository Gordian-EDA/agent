...
/// The optimistic-concurrency hook: a mutator handed a revision the project has
/// moved past refuses, and says who moved it and what they touched.
#[test]
fn a_stale_expect_revision_is_refused_with_the_current_writer() {
    let Some(ctx) = AgentRuntime::detect_for_test() else {
        eprintln!("SKIP: no KiCAD detected");
        return;
    };
    ...
    let stale = tool(&ctx, "checkpoint", json!({ "label": "before placing" }))["revision"]
        .as_u64()
        .unwrap();
    ...
    let refused = run_tool(
        "lock_parts",
        json!({ "refs": ["R1"], "expect_revision": stale }),
        &ctx,
    )
    .unwrap();
    assert_eq!(refused["code"], json!("revision_conflict"), "{refused:#}");
    assert_eq!(refused["expected_revision"], json!(stale), "{refused:#}");
    assert_eq!(refused["current_tool"], json!("place_board"), "{refused:#}");
    assert!(
        refused["refs_touched"]
            .as_array()
            .is_some_and(|refs| !refs.is_empty()),
        "the conflict names what the other writer touched: {refused:#}"
    );

    // The same call against the current revision goes through.
    let current = run_tool("history", json!({ "limit": 1 }), &ctx).unwrap();
    ...
    let locked = tool(
        &ctx,
        "lock_parts",
        json!({ "refs": ["R1"], "expect_revision": current }),
    );
    assert_eq!(locked["locked_reason"], json!("agent"), "{locked:#}");
}
