...
Build in order: power entry; regulator; MCU core with every supply-pin decoupler, crystal, reset and boot straps; interfaces; connectors and indicators. Use `place_parts({parts, layout, name?, intent?, block?, blocks?})` for only the current block, then use `connect`, `label`, `no_connect`, and `arrange({refs|block|region, layout})` for focused corrections. Include support, protection, decoupling, bias, termination and indicator resistors. State real symbol `Lib:Name`s, values, footprints and pin-to-net maps. Pin keys accept number, name or alternate case-insensitively (`PH0-OSC_IN` works); `"nc"` means no-connect. Rails and ports accept left, right, top or bottom. YOU compose the layout, the engine only measures: pass `layout: {<block>: <tree>}` for every block. A node is `{part, unit?, rot?, mirror?}`, `{row: [...]}` or `{col: [...]}`, with `gap` (grid units; 1 = 1.27 mm, an 0603 resistor is 6). A row is ONE signal path: neighbours in a row must share a net, so they get a straight wire. Never put unrelated parts side by side. Whatever hangs off a node (shunt cap, pull-up, bias resistor) goes in a `col` with the series part it attaches to. An IC sits between a col of input-side parts and a col of output-side parts, with its decoupling caps in a row after it. Parts meeting only through a rail need no adjacency; symmetric halves are two mirrored cols in one row. 3-12 parts per block, every part placed, gaps 4-6 in a passive chain and 6-8 around an IC. Omit `rot` unless the default is wrong (series passives lie along their row, a part on a rail stands GND-down, an end connector faces the circuit). Always pass `name` (the sheet's title) and give each `block` a descriptive name; add `blocks: {<block>: {title?, note?}}` with a one-line `note` wherever a human would explain a decision the netlist cannot show: a switching frequency, a sizing choice, why a pull-up is on this side.
...
`dangling` pins are reported, not fatal: close each with `connect`, or declare real board I/O in `intent.ports`. Write `"@R1.2"` as a net to join that pin's net. Resolve `completeness.gaps` with one follow-up `place_parts` of only the missing parts; gaps are advisory for deliberately minimal designs.
...
`check_schematic` reports every finding. Fix ERC errors in what you touched; mention unrelated existing ones and leave them. Once it reports 0 ERC errors, proceed to the board in the SAME turn. Address ERC warnings and appearance only after the board is routed and DRC-clean; never spend a whole turn on warnings. Missing footprints remain staged work.
...
5. Route critical nets first: power, crystal, differential pairs. `route_board` keeps every clean routed net, including partial plane fanout, and reports unreached plane pads in the ratsnest. If blocked, inspect the returned blocker and alternatives; `route_track` raises a narrow width to the board minimum and drops a same-layer via. `move_parts` nudges an occupied target to `nudged_to`. Adjust placement/copper and re-route ONLY the blocked nets; consider swapping header/GPIO pins, then re-check and sync.
