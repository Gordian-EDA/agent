...
use sch_floorplan::floorplan::place_problem;
...
"#,
    ...
    let mut problem = place_problem(&env, &design, None, PlaceOptions::default()).unwrap();

    let (inc, ir) = (problem.inc.clone(), problem.ir.clone());
    let realizer = sch_floorplan::floorplan::place::RoutedSheetRealizer::new(&env, &inc, &ir);
    let eval = sch_floorplan::floorplan::place::RoutedEvaluator::new(realizer, &design);
    let output = SpinePlace.place(&mut problem, &eval);
