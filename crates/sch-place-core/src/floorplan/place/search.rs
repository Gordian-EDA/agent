//! `place::search` — the PREMIUM simulated-annealing placement SEARCH. The
//! env-free move-set + proxy costs live here alongside the scaffold they drive
//! (cohesion/anchor/idiom-align/decongest in [`super::idioms`]/[`super::refine`]),
//! so the whole search is in one home; `anneal-place` is then a thin
//! [`sch_model::place::PlacementEngine`] that calls [`anneal_place`].
//!
//! SCORING goes entirely through the injected [`PlacementCost`] — the search never
//! imports the routed scorer directly nor touches a KiCAD CLI — so it stays a pure
//! geometry/proxy search over the cost boundary, exactly as the engine crate drove
//! it before this consolidation.

use std::collections::{BTreeMap, BTreeSet};

use super::*;
use crate::write::pin_endpoint;
use sch_model::item::{Incidence, Item};
use sch_model::ir::{LayoutIr, Orient};
use sch_model::netclass::is_power_net;
use sch_model::place::{Crossings, PlaceProblem, PlaceResult, PlacementCost};
...
pub fn anneal_place(p: &PlaceProblem, items: &mut [Item], engine: &'static str) -> PlaceResult {
    ...
    if items.is_empty() {
        return report(engine, cost, items);
    }
