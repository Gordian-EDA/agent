//! Contract tests for the standard DRC oracle's report and its copper-cleanup
//! passes: every rule's finding shape, and the drop passes that keep an emitted
//! solution geometry-clean and connectivity-honest.

use pcb_drc::StandardDrc;
use pcb_model::{Drc, Finding, Violation};
    ...
    fn via_diameter_below_min_flags_undersized_through_but_allows_micro() {
        ...
        let flagged = |s: &RouteSolution| {
            StandardDrc.check(&p, s)
                .iter()
                .any(|v| matches!(v, Finding::ViaDiameterBelowMin { .. }))
        };
    ...
    fn drop_violating_copper_drops_undersized_via_net() {
        ...
        let dropped = StandardDrc.drop_violating_copper(&p, &mut sol);
        assert_eq!(dropped, vec!["VCC".to_owned()]);
        assert!(sol.vias.is_empty(), "undersized via dropped, not shipped");
    }

    fn count<F: Fn(&Finding) -> bool>(vs: &[Finding], f: F) -> usize {
    ...
    fn clearance_trace_trace_fires_for_close_parallel_traces() {
        ...
        let vs = StandardDrc.check(&p, &s);
        assert_eq!(
            count(&vs, |v| matches!(
                v,
                Finding::ClearanceTraceTrace { .. }
            )),
            1,
    ...
    fn clearance_trace_trace_clean_for_far_parallel_traces() {
        ...
        assert!(
            !StandardDrc.check(&p, &s)
                .iter()
                .any(|v| matches!(v, Finding::ClearanceTraceTrace { .. })),
    ...
    fn clearance_trace_obstacle_clean_for_own_pad() {
        ...
        assert!(
            !StandardDrc.check(&p, &s)
                .iter()
                .any(|v| matches!(v, Finding::ClearanceTraceObstacle { .. })),
    ...
    fn clearance_via_any_fires_for_foreign_trace() {
        ...
        assert_eq!(
            count(&vs, |v| matches!(
                v,
                Finding::ClearanceViaAny { required, .. } if *required < 0.24
            )),
            1,
    ...
    fn hole_clearance_fires_for_close_drills() {
        ...
        let hole = count(&vs, |v| {
            matches!(
                v,
                Finding::ClearanceViaAny { required, .. } if (*required - 0.25).abs() < 1e-9
            )
        });
        assert_eq!(
            hole, 1,
    ...
    fn hole_clearance_fires_for_via_near_foreign_pad() {
        ...
        let hole = count(&StandardDrc.check(&p, &s), |v| {
            matches!(
                v,
                Finding::ClearanceViaAny { required, .. } if (*required - 0.25).abs() < 1e-9
            )
        ...
        let hole2 = count(&StandardDrc.check(&p2, &s2), |v| {
            matches!(
                v,
                Finding::ClearanceViaAny { required, .. } if (*required - 0.25).abs() < 1e-9
            )
    ...
    fn copper_edge_clearance_fires_for_via_near_custom_outline() {
        ...
        assert!(
            StandardDrc.check(&p, &near)
                .iter()
                .any(|v| matches!(v, Finding::OutOfBounds { .. })),
    ...
    fn clearance_via_any_clean_for_own_trace() {
        ...
        assert!(
            !StandardDrc.check(&p, &s)
                .iter()
                .any(|v| matches!(v, Finding::ClearanceViaAny { .. })),
    ...
    fn out_of_bounds_fires_for_via_off_board() {
        ...
        let vs = StandardDrc.check(&p, &s);
        assert_eq!(
            count(&vs, |v| matches!(v, Finding::OutOfBounds { .. })),
            1,
    ...
    fn out_of_bounds_fires_for_trace_off_board() {
        ...
        let vs = StandardDrc.check(&p, &s);
        assert!(
            count(&vs, |v| matches!(v, Finding::OutOfBounds { .. })) >= 1,
    ...
    fn invalid_layer_fires_for_inner1_on_two_layer_board() {
        ...
        let vs = StandardDrc.check(&p, &s);
        let invalid_count = count(&vs, |v| matches!(v, Finding::InvalidLayer { .. }));
        assert_eq!(
            invalid_count, 2,
        ...
        let trace_viol = vs.iter().find(|v| {
            matches!(
                v,
                Finding::InvalidLayer { layer, layer_count: 2, .. }
                    if layer == "inner1"
            )
        });
        assert!(
    ...
    fn invalid_layer_does_not_fire_for_valid_layers() {
        ...
        assert!(
            !vs.iter()
                .any(|v| matches!(v, Finding::InvalidLayer { .. })),
